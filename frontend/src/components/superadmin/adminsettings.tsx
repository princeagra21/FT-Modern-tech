...
function getPart(date: Date, locale: string, tz: string, opts: Intl.DateTimeFormatOptions) {
  return new Intl.DateTimeFormat(locale, { timeZone: tz, ...opts }).format(date);
}

function formatDateByPattern(date: Date, locale: string, tz: string, pattern: AdminSettings["dateFormatId"]) {
...
function formatPreviewDate(date: Date, locale: string, tzOffset: string, dateFormatId: AdminSettings["dateFormatId"], use24h: boolean) {
  // Parse offset string like "+05:30" or "-08:00"
  const match = tzOffset.match(/([+-])(\d{2}):(\d{2})/);
  if (!match) {
    const timeOpts: Intl.DateTimeFormatOptions = { hour: "2-digit", minute: "2-digit", hour12: !use24h };
  ...
  const timeOpts: Intl.DateTimeFormatOptions = { hour: "2-digit", minute: "2-digit", hour12: !use24h, timeZone: "UTC" };
  const dateStr = formatDateByPattern(adjustedDate, locale, "UTC", dateFormatId);
  const timeStr = new Intl.DateTimeFormat(locale, timeOpts).format(adjustedDate);
  return { dateStr, timeStr };
}
...
export default function AdminSettingPage() {
  ...
  const now = new Date();
  const preview = formatPreviewDate(now, settings.language, settings.timezone, settings.dateFormatId, settings.time24h);
  ...
  const onReset = () => { setSettings(DEFAULTS); saveSettings(DEFAULTS); };
    ...
    <div className="mx-auto max-w-7xl px-6 py-8">
      ...
      <div className="mb-6 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="flex h-11 w-11 items-center justify-center rounded-2xl border border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800">
            <SettingsIcon className="dark:text-neutral-300" />
          </motion.div>
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-100">Admin Setting</h1>
            <p className="text-sm text-neutral-500 dark:text-neutral-400">Premium static settings for language, layout, formats, theme, timezone & units.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={onReset} className="rounded-xl border-neutral-300 dark:border-neutral-600 text-neutral-900 dark:text-neutral-300 hover:bg-white dark:hover:bg-neutral-700"><RestoreIcon className="mr-2"/> Reset</Button>
          <Button onClick={onSave} className="rounded-xl bg-neutral-900 dark:bg-white text-white dark:text-black hover:bg-black dark:hover:bg-white/90"><SaveIcon className="mr-2"/> Save</Button>
        </div>
      </div>
      ...
      <div>
          <div className={classNames(
            ...
            <div className="mb-3 flex items-center justify-between">
              ...
              <div className="text-xs opacity-60">Theme: {settings.theme.toUpperCase()} • Dir: {settings.direction.toUpperCase()} • Units: {settings.units.toUpperCase()}</div>
            </div>
            <Separator className={isDark ? "bg-neutral-800" : "bg-neutral-200"} />
            <div dir={settings.direction} className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <div className="text-xs opacity-60">Date</div>
                <div className="text-lg font-medium">{preview.dateStr}</div>
              </div>
              <div>
                <div className="text-xs opacity-60">Time</div>
                <div className="text-lg font-medium">{preview.timeStr}</div>
                <div className="text-xs opacity-60 mt-1 font-mono">UTC {settings.timezone}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 mt-3 gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Card title="Language" icon={<TranslateIcon />}>
          <Label className="mb-2 block text-neutral-700 dark:text-neutral-300">Choose language</Label>
          <Select value={settings.language} onValueChange={(v) => set("language", v)}>
            <SelectTrigger className="rounded-xl border-neutral-300 dark:border-neutral-600 text-neutral-900 dark:text-neutral-200 dark:bg-neutral-800 focus-visible:ring-neutral-900 dark:focus-visible:ring-neutral-400">
              <SelectValue />
            </SelectTrigger>
        ...
        <Card title="Date Format" icon={<TodayIcon />}>
          <Label className="mb-2 block text-neutral-700 dark:text-neutral-300">Display style</Label>
          <Select value={settings.dateFormatId} onValueChange={(v) => set("dateFormatId", v as AdminSettings["dateFormatId"]) }>
            <SelectTrigger className="rounded-xl border-neutral-300 dark:border-neutral-600 text-neutral-900 dark:text-neutral-200 dark:bg-neutral-800 focus-visible:ring-neutral-900 dark:focus-visible:ring-neutral-400">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="rounded-2xl border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800">
        ...
        <Card title="Time Format" icon={<AccessTimeIcon />}>
          <div className="flex items-center justify-between rounded-xl border border-neutral-300 dark:border-neutral-600 dark:bg-neutral-900/50 p-3">
            <span className="text-sm text-neutral-800 dark:text-neutral-300">24-hour clock</span>
            <Switch checked={settings.time24h} onCheckedChange={(v) => set("time24h", Boolean(v))} />
          </div>
        </Card>
        ...
        <Card title="Timezone" icon={<PublicIcon />}>
          <Label className="mb-2 block text-neutral-700 dark:text-neutral-300">UTC Offset</Label>
          <Select value={settings.timezone} onValueChange={(v) => set("timezone", v)}>
            <SelectTrigger className="rounded-xl border-neutral-300 dark:border-neutral-600 text-neutral-900 dark:text-neutral-200 dark:bg-neutral-800 focus-visible:ring-neutral-900 dark:focus-visible:ring-neutral-400">
              <SelectValue>
                <span className="font-mono font-semibold text-lg dark:text-neutral-200">{settings.timezone}</span>
              </SelectValue>
            </SelectTrigger>
            <SelectContent className="max-h-72 overflow-auto rounded-2xl border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800">
              {TIMEZONE_OFFSETS.map((tz) => (
                <SelectItem key={tz.value} value={tz.value}>
                  <span className="font-mono font-semibold text-base">{tz.label}</span>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <Card title="First Day of Week" icon={<TodayIcon />}>
          <Select value={settings.firstDayOfWeek} onValueChange={(v) => set("firstDayOfWeek", v)}>
            <SelectTrigger className="rounded-xl border-neutral-300 dark:border-neutral-600 text-neutral-900 dark:text-neutral-200 dark:bg-neutral-800 focus-visible:ring-neutral-900 dark:focus-visible:ring-neutral-400">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="rounded-2xl border-neutral-200 dark:border-neutral-700 dark:bg-neutral-800">
...
function Card({ title, icon, children }: { title: string; icon?: React.ReactNode; children: React.ReactNode }) {
  return (
    <motion.div 
      ...
      className="rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-5 shadow-sm hover:shadow-md transition-shadow"
    >
      <div className="mb-4 flex items-center gap-2.5">
        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-neutral-50 dark:bg-neutral-700 border border-neutral-200 dark:border-neutral-600 text-neutral-700 dark:text-neutral-300">{icon}</div>
